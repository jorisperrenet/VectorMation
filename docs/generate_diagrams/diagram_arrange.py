#!/usr/bin/env python3
"""Generate arrange / distribute layout diagram."""
...
# --- arrange (top half) ---
canvas.add_objects(Text("arrange(direction='right', buff=12)", x=125, y=22, font_size=12, fill='#cdd6f4'))
...
for i, ((w, h), c) in enumerate(zip(sizes, colors)):
    canvas.add_objects(Rectangle(w, h, x=x, y=y_top,
...
# --- arrange down (middle) ---
canvas.add_objects(Text("arrange(direction='down')", x=155, y=112, font_size=12, fill='#cdd6f4'))

y = 130
col_x = 200
for i, ((w, h), c) in enumerate(zip(sizes[:3], colors[:3])):
    canvas.add_objects(Rectangle(w, h, x=col_x, y=y,
                                 fill=c, fill_opacity=0.25, stroke=c, stroke_width=1.8))
    y += h + buff

# --- arrange_in_grid (right side) ---
canvas.add_objects(Text("arrange_in_grid(rows=2, cols=2)", x=290, y=112, font_size=12, fill='#cdd6f4'))
...
for row in range(2):
    for col in range(2):
        ...
        canvas.add_objects(Rectangle(grid_w, grid_h, x=gx, y=gy,
                                     fill=colors[idx], fill_opacity=0.25, stroke=colors[idx], stroke_width=1.8))

out = os.path.join(os.path.dirname(__file__), '..', 'source', '_static', 'images', 'arrange.svg')
