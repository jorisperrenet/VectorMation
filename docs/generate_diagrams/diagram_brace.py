#!/usr/bin/env python3
"""Generate brace parameter diagram."""
...
canvas.add_objects(target)
canvas.add_objects(Text('target object', x=148, y=112, font_size=13, fill='#58C4DD'))

# Brace below the target
brace = Brace(target, direction='down', label='label', buff=10,
              stroke='#cdd6f4', stroke_width=2, fill='#f9e2af')
canvas.add_objects(brace)

# buff annotation (right side)
rect_bottom = 105 + 30  # y + height/2
canvas.add_objects(
    ...
    Arrow(x1=370, y1=rect_bottom + 9, x2=370, y2=rect_bottom, tip_length=3, tip_width=3, stroke='#a6e3a1', stroke_width=0),
    Text('buff', x=380, y=rect_bottom + 8, font_size=11, fill='#a6e3a1'),
)

# direction annotation (left side)
canvas.add_objects(
...
out = os.path.join(os.path.dirname(__file__), '..', 'source', '_static', 'images', 'brace_params.svg')
