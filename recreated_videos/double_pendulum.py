...
title = Text(text='Double Pendulum: Chaos', x=960, y=60,
             font_size=42, fill='#fff', stroke_width=0, text_anchor='middle')
title.fadein(0, 0.5)

subtitle = Text(text='Nearly identical initial conditions diverge rapidly',
...
for i, cfg in enumerate(configs):
    lx = 100
    ly = 900 + i * 35
    all_objects.append(Circle(r=8, cx=lx, cy=ly, fill=cfg['color'], stroke_width=0))
    angle_str = f"{cfg['theta1']:.2f} rad"
    all_objects.append(Text(text=f"Pendulum {cfg['label']}: \u03b81 = {angle_str}",
                           x=lx + 20, y=ly + 6, font_size=20, fill=cfg['color'],
                           stroke_width=0))
