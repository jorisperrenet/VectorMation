...
v.add(Rectangle(PW, PH, x=PX, y=PY,
...
for i in range(100):
    t = -2 + 5 * i / 99
    val = math.sin(t * 1.5) * 1.5
    px_pt = PX + (t + 2) / 5 * PW
    py_pt = PY + PH - (val + 2) / 5 * PH
    pts.append((px_pt, py_pt))
v.add(Lines(*pts, stroke='#a6e3a1', stroke_width=2, fill_opacity=0))
...
v.add(Text('ticks', x=PX + PW - 30, y=PY + PH + 18, font_size=9, fill='#585b70'))
v.add(Text('plot(func)', x=PX + PW - 70, y=PY + 35, font_size=11, fill='#a6e3a1'))
